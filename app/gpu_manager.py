...
class CUDAGPUManager:
    ...
    def __init__(
        ...
        monitoring_interval: float = 300.0,  # 5 minutes default
        ...
        Args:
            ...
            monitoring_interval: Monitoring interval in seconds (default: 300 = 5 minutes)
    ...
    def start_monitoring(self):
        ...
        if hasattr(self, "config") and self.config:
            ...
            # Check GPU config section
            if hasattr(self.config, "gpu"):
                logger.info(f"Config has gpu section: {self.config.gpu}")
                if hasattr(self.config.gpu, "enable_monitoring"):
                    ...
                    if not self.config.gpu.enable_monitoring:
                        ...
                        return
                if hasattr(self.config.gpu, "monitoring_interval"):
                    logger.info(
                        f"gpu.monitoring_interval value: {self.config.gpu.monitoring_interval}"
                    )
                    self.monitoring_interval = self.config.gpu.monitoring_interval
            else:
                logger.info("Config does not have gpu section")

            # Check monitoring config section (legacy support)
            if hasattr(self.config, "monitoring"):
                logger.info(f"Config has monitoring section")
        ...
        logger.info(
            f"GPU memory monitoring started with interval: {self.monitoring_interval}s"
        )
...
def get_gpu_manager(config=None) -> CUDAGPUManager:
    ...
    if gpu_manager is None:
        # Extract monitoring interval from config if available
        monitoring_interval = 300.0  # Default to 5 minutes
        if (
            config
            and hasattr(config, "gpu")
            and hasattr(config.gpu, "monitoring_interval")
        ):
            monitoring_interval = config.gpu.monitoring_interval

        gpu_manager = CUDAGPUManager(monitoring_interval=monitoring_interval)

    # Always update config if provided (even for existing instances)
    if config:
        gpu_manager.config = config
        # Update monitoring interval if specified in config
        if hasattr(config, "gpu") and hasattr(config.gpu, "monitoring_interval"):
            gpu_manager.monitoring_interval = config.gpu.monitoring_interval
