...
class LLMOptimized:
    ...
    - LLaVA 1.6 multimodal support
    """

    # Model context sizes optimized for LLaVA 1.6
    TEXT_MODEL_CONTEXT_SIZE = 4096
    VISION_MODEL_CONTEXT_SIZE = 4096  # Increased to match text model
    MAX_ALLOWED_OUTPUT_TOKENS = 2048

    # LLaVA 1.6 specific settings
    LLAVA_VISION_TOKENS = 576  # Standard LLaVA image token count
    LLAVA_IMAGE_TAG = "<image>"  # LLaVA 1.6 image placeholder

    def __init__(self, settings: LLMSettings = None):
        """Initialize the optimized LLM with GPU management and LLaVA 1.6 support."""
        ...
        # GPU management with configuration - use global GPU manager
        self.gpu_manager = get_gpu_manager(config)

        # Apply force_cuda and force_gpu_layers from config if available
        gpu_config = getattr(config, "gpu", None)
        if gpu_config:
            if hasattr(gpu_config, "force_cuda"):
                self.gpu_manager.force_cuda = gpu_config.force_cuda
            if hasattr(gpu_config, "force_gpu_layers"):
                self.gpu_manager.force_gpu_layers = gpu_config.force_gpu_layers
        ...
        self.vision_settings = self.settings.vision if self.settings.vision else None
        self.vision_enabled = self._validate_vision_model()

        # Check if using LLaVA model for both text and vision
        self.is_llava_model = self._detect_llava_model()
        ...
        logger.info(
            ...
            f"Vision: {self.vision_enabled}, LLaVA: {self.is_llava_model}"
        )
    ...
    def _detect_llava_model(self) -> bool:
        """Detect if this is a LLaVA model based on model name or path."""
        model_indicators = ["llava", "vision", "multimodal"]
        model_name_lower = self.model.lower()
        model_path_lower = self.model_path.lower()

        return any(
            indicator in model_name_lower or indicator in model_path_lower
    ...
    def _load_text_model(self):
        """Load text model with optimized GPU settings and timeout protection."""
        if self._text_model_key not in MODEL_CACHE:
            ...
            if use_gpu:
                # Estimate total layers (rough estimate for common models)
                estimated_layers = int(model_size_gb * 10)
            ...
            # Try GPU first, with timeout protection using threading
            model = None
            if gpu_layers > 0:
                try:
                    import threading
                    ...
                    logger.info("Attempting GPU model loading with 60s timeout...")
                    ...
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        future = executor.submit(load_gpu_model)
                        try:
                            ...
                            logger.warning(
                                "GPU model loading timed out after 60 seconds"
                            )
                            model = None
                except Exception as e:
                    # Only log as warning, do not raise or exit
                    logger.warning(f"GPU model loading failed: {e}")
                    model = None

            # Fallback to CPU-only if GPU failed or wasn't attempted
            if model is None:
                ...
                try:
                    ...
                    load_time = time.time() - start_time
                    logger.info(
                        f"Text model loaded in CPU-only mode in {load_time:.2f}s"
                    )
                except Exception as e:
                    logger.error(f"Failed to load text model even in CPU mode: {e}")
                    # Do not raise, just log error and return None
                    return None
        return MODEL_CACHE.get(self._text_model_key)
    ...
    def _load_vision_model(self):
        ...
        if self._vision_model_key not in MODEL_CACHE:
            ...
            try:
                ...
                # Try GPU first with timeout protection
                model = None
                if gpu_layers > 0:
                    try:
                        from concurrent.futures import ThreadPoolExecutor
                        ...
                        def load_gpu_vision_model():
                            return Llama(
                                ...
                                n_threads=min(
                                    os.cpu_count(), 4
                                ),  # Fewer threads for vision
                                use_mmap=True,
                                use_mlock=False,
                                verbose=False,
                            )

                        # Use a separate thread pool with timeout
                        with ThreadPoolExecutor(max_workers=1) as executor:
                            future = executor.submit(load_gpu_vision_model)
                            try:
                                model = future.result(
                                    timeout=45
                                )  # 45 second timeout for vision

                            except FutureTimeoutError:
                                logger.warning(
                                    "Vision model GPU loading timed out after 45 seconds"
                                )
                                model = None
                    except Exception as e:
                        logger.warning(f"Vision model GPU loading failed: {e}")
                        model = None

                # Fallback to CPU if GPU failed
                if model is None and gpu_layers > 0:
                    logger.warning("Falling back to CPU-only mode for vision model")
                    try:
                        model = Llama(
                            model_path=self.vision_settings.model_path,
                            n_ctx=self.VISION_MODEL_CONTEXT_SIZE,
                        ...
                        logger.error(
                            f"Failed to load vision model even in CPU mode: {e}"
                        )
                        raise
                elif model is None:
                    # Direct CPU loading if no GPU was attempted
                    model = Llama(
                        model_path=self.vision_settings.model_path,
                        n_ctx=self.VISION_MODEL_CONTEXT_SIZE,
                        n_gpu_layers=0,
    ...
    def _format_vision_prompt(self, messages: List[Dict[str, Any]]) -> str:
        """Format messages for vision models, handling image content with LLaVA 1.6 support."""
        prompt = ""
        for message in messages:
            role = message["role"]
            content = message.get("content", "")
                    ...
                    for item in content:
                        if isinstance(item, dict):
                            if item.get("type") == "text":
                                ...
                                # LLaVA 1.6 specific image handling
                                if self.is_llava_model:
                                    prompt += f"{self.LLAVA_IMAGE_TAG}\n"
                                else:
                                    # Generic vision model placeholder
                                    prompt += "[IMAGE]\n"
            elif role == "assistant":
                prompt += f"<|assistant|>\n{content}\n"
            else:
    ...
    def count_message_tokens(self, messages: List[Dict[str, Any]]) -> int:
        """
        Count the number of tokens in a list of messages with LLaVA 1.6 support.
                ...
                for item in content:
                    if isinstance(item, dict):
                        if item.get("type") == "text":
                            ...
                            # Add image tokens for LLaVA 1.6
                            if self.is_llava_model:
                                total_tokens += self.LLAVA_VISION_TOKENS
                            else:
                                # Generic vision model estimate
...
You can also provide regular text responses. If you need to use a tool, include the tool call JSON in your response.""",
                    ...
                    if tools and any(
                        ...
                        try:
                            ...
                            tool_call = ToolCall(
                                ...
                                type="function",
                                function=Function(
                                    name=func_name, arguments=json.dumps(arguments)
                                ),
                            )
                            tool_calls.append(tool_call)
                            ...
                            logger.warning(
                                f"Failed to parse function call {func_name}: {e}"
                            )
                            continue
            ...
            return {"content": f"[Unexpected error: {str(e)}]", "tool_calls": []}
