...
class BaseAgent(BaseModel, ABC):
    ...
    async def step(self) -> str:
        """Execute a single step in the agent's workflow, with LLM None check."""
        if (
            not hasattr(self, "llm")
            or self.llm is None
            or getattr(self.llm, "model", None) is None
        ):
            logger.error(
                "LLM model is not loaded or failed to initialize. Cannot proceed with step."
            )
            return "Error: LLM model is not loaded or failed to initialize. Please check your model configuration and logs."
        return "Step not implemented in base agent."
